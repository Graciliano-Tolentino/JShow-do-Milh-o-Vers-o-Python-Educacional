...
class ShowDoMilhaoApp:
    ...
    def nova_pergunta(self):
        ...
        tk.Button(self.frame, text="Dica", command=self.usar_dica).pack(side='left', padx=10, pady=20)
        tk.Button(self.frame, text="Pular", command=self.usar_pular).pack(side='left', padx=10)
        tk.Button(self.frame, text="Eliminar Duas", command=self.usar_eliminar).pack(side='left', padx=10)
    ...
    def usar_eliminar(self):
        if self.ajudas['eliminar'] > 0:
            ...
            restantes = [op for op in self.pergunta_atual['opcoes'] if op not in eliminadas]
            messagebox.showinfo("Eliminadas", f"As opções eliminadas foram: {', '.join(eliminadas)}")
        else:
