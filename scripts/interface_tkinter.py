...
# Caminho absoluto para o JSON (arquivo na raiz do projeto)
CAMINHO_JSON = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'perguntas_show_do_milhao.json'))

# Carregar perguntas com verificação de erro
try:
...
class ShowDoMilhaoApp:
    ...
    def nova_pergunta(self):
        ...
        tk.Label(
            self.frame,
            text=f"Pergunta {self.rodada}: {self.pergunta_atual['pergunta']}",
            wraplength=500
        ).pack(pady=10)
    ...
    def usar_eliminar(self):
        if self.ajudas['eliminar'] > 0:
            ...
            erradas = [op for op in self.pergunta_atual['opcoes'] if op != correta]
